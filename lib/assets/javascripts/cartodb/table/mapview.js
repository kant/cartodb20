...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  */
  enableMap: function() {
    ...
    if (!this.map_enabled) {
      ...
      this.mapView.$el.append(self.infowindow.el);
      */
      ...
      this.geometryEditor.bind('geomCreated', function(row) {
        this.options.table.data().add(row);
      }, this);

      self.infowindow.bind('editGeom', this.geometryEditor.editGeom, this.geometryEditor);
      this.options.table.bind('remove:row', this.updateDataLayerView, this);
  ...
  */
  showNoGeoRefWarning: function() {
    ...
    if(this.options.table._data.length > 0) {
      warningStorageName = 'georefContentWarningShowed' + this.options.table.id;
    } else {
      warningStorageName = 'georefNoContentWarningShowed' + this.options.table.id;
    }
  ...
  _addSQLViewHeader: function() {
    this.$('.sqlview').remove();
    var total = this.options.table.data().size();
