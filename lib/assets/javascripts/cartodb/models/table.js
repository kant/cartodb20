...
(function() {
  ...
  cdb.admin.CartoDBTableData = cdb.ui.common.TableData.extend({
    ...
    _quantificationMethod: function(functionName, nslots, column, callback, error) {
      var tmpl = _.template('select unnest(<%= functionName %>(array_agg(<%= column %>::numeric), <%= slots %>)) as buckets from <%= table_name %> where <%= column %> is not null');
      this._sqlQuery(tmpl({
        slots: nslots,
        table_name: this.table.get('name'),
        column: column,
    ...
    jenkBins: function(nslots, column, callback, error) {
      this._quantificationMethod('CDB_JenksBins', nslots, column, callback, error);
    },

    headTails: function(nslots, column, callback, error) {
      this._quantificationMethod('CDB_HeadsTailsBins', nslots, column, callback, error);
    },

    quantileBins: function(nslots, column, callback, error) {
      this._quantificationMethod('CDB_QuantileBins', nslots, column, callback, error);
    },
